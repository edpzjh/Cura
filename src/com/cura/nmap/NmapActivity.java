...
public class NmapActivity extends Activity {
 ...
 /*
  * --- program variables - these need to be backed up for configuration changes ---
  */
 ...
 public void onCreate(Bundle savedInstanceState) {
  ...
  ArrayAdapter<CharSequence> adapterCommand = ArrayAdapter.createFromResource(this, R.array.commands, android.R.layout.simple_spinner_item);
  ...
  mCommandSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
   ...
   public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
	...
	Log.d(tag, mCommandSpinner.getSelectedItem().toString() + " selected.");
   }
  ...
  ArrayAdapter<CharSequence> adapterOutput = ArrayAdapter.createFromResource(this, R.array.output, android.R.layout.simple_spinner_item);
  ...
  mStart.setOnClickListener(new View.OnClickListener() {
   ...
   public void onClick(View v) {
	...
	mResults.setText("");
	saveResults.setVisibility(Button.INVISIBLE);
	String s = mTarget.getText().toString() + " " + mArguments.getText().toString();
	if(s == null || s.length() == 0)
	 s = "";
	if(sTask == null || sTask.getStatus() == AsyncTask.Status.FINISHED || sTask.getStatus() == AsyncTask.Status.RUNNING) {
  ...
  saveResults.setOnClickListener(new View.OnClickListener() {
   ...
   public void onClick(View v) {
	...
	try {
	 ...
	 String fileName = user.getUsername() + "_" + mCommandSpinner.getSelectedItem().toString() + "_" + mOutputSpinner.getSelectedItem().toString()
	   + "_" + dateString + ".txt";
	 ...
	 target.flush();
	 target.close();
	 Toast.makeText(NmapActivity.this, getString(R.string.nmapResultsSaved) + " \"/Nmap/" + fileName + "\"", Toast.LENGTH_LONG).show();
	}
	catch (Exception e) {
	 // TODO Auto-generated catch block
	 Toast.makeText(NmapActivity.this, R.string.resultsNotSaved, Toast.LENGTH_LONG).show();
	}
   }
  });
 }
 ...
 public boolean onCreateOptionsMenu(Menu menu) {
  ...
  menu.add(0, HELP, 0, R.string.Help).setIcon(android.R.drawable.ic_menu_help);
  return result;
 }
 ...
 private class scan extends AsyncTask<String, Void, Void> {
  ...
  protected void onPreExecute() {
   ...
   Log.d(tag, "Selected scan type: " + command + " (" + NmapActivity.scanType + ")");
 ...
 private class verifyInstallation extends AsyncTask<Void, Void, Void> {
  ...
  private ProgressDialog pd;
  private String filenames[] = { "ndiff", "nping", "nmap-services", "nmap-mac-prefixes" };
  ...
  protected void onPreExecute() {
   ...
   if(canGetRoot)
	...
	Log.d(tag, "su NOT found - will attempt to run without root permissions.");
  ...
  protected Void doInBackground(Void... params) {
   try {
	...
	File instdir = new File(NmapActivity.this.getPackageManager().getApplicationInfo("com.cura", 0).dataDir + "/install/");
	NmapActivity.outputArgs = new String(" -oA " + NmapActivity.this.getPackageManager().getApplicationInfo("com.cura", 0).dataDir + "/tmp/scan ");

	Log.d(tag, "Will attempt to uncompress binaries to " + instdir.getCanonicalPath());
	...
	File tmpdir = new File(NmapActivity.this.getPackageManager().getApplicationInfo("com.cura", 0).dataDir + "/tmp/");
	...
	// create.
	Process p = canGetRoot ? Runtime.getRuntime().exec("su") : Runtime.getRuntime().exec("sh");
	DataOutputStream os = new DataOutputStream(p.getOutputStream());
	BufferedReader in = new BufferedReader(new InputStreamReader(p.getInputStream()));
	BufferedReader err = new BufferedReader(new InputStreamReader(p.getErrorStream()));

	int requiredResources[] = { R.raw.ndiff, R.raw.nping, R.raw.nmap_services, R.raw.nmap_mac_prefixes };
	...
	for (int k = 0; k < requiredResources.length; k++) {
	 ...
	 if(!(new File(filenames[k]).exists())) {
	  InputStream in1 = getResources().openRawResource(requiredResources[k]);
	  OutputStream out = new FileOutputStream(instdir + "/" + filenames[k]);
	  ...
	  in1.close();
	  out.close();
	  os.writeBytes("cat " + instdir.toString() + "/" + filenames[k] + " > " + bindir + filenames[k] + "\n");
	  Log.d(tag, "cat " + instdir.toString() + "/" + filenames[k] + " > " + bindir + filenames[k]);
	  if(canGetRoot) {
	...
	try {
	 ...
	 if(type.equals("Normal"))
	  ...
	  if(type.equals("XML"))
	   ...
	   if(type.equals("Grepable"))
		type = new String("gnmap");
	   else {
		Log.d(tag, "Something went wrong with mOutputSpinner and received type: " + type);
		type = new String("nmap");
	   }
	 BufferedReader b = new BufferedReader(new FileReader(NmapActivity.this.getPackageManager().getApplicationInfo("com.cura", 0).dataDir
	   + "/tmp/scan." + type));
	 String l;
 ...
 protected void onRestoreInstanceState(Bundle savedInstanceState) {
  ...
  installationVerified = savedInstanceState.getBoolean("installationVerified");
 ...
 public boolean onKeyDown(int keyCode, KeyEvent event) {
  if((keyCode == KeyEvent.KEYCODE_BACK)) {
   ...
   // confirm logout
	 .setMessage(R.string.logoutConfirmationDialog).setPositiveButton("Yes", new DialogInterface.OnClickListener() {
	  ...
	  public void onClick(DialogInterface dialog, int which) {
	   ...
	   Intent closeAllActivities = new Intent(NmapActivity.this, LoginScreenActivity.class);
