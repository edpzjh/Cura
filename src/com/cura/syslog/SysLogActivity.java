...
public class SysLogActivity extends Activity implements android.view.View.OnClickListener {

 private String menu1[] = new String[] { "Head", "Tail" };
 private String menu2[] = new String[] { "errors.log", "kernel.log", "boot", "auth.log", "daemon.log", "dmesg.log", "crond.log", "user.log",
   "Xorg.0.log" };
 ...
 public void initSpinners() {
  ...
  ArrayAdapter<String> adapter1 = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, menu1);
  adapter1.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
  position.setAdapter(adapter1);

  logFile = (Spinner) findViewById(R.id.LogFileSpinner);
  ArrayAdapter<String> adapter2 = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, menu2);
   ...
   protected Boolean doInBackground(String... params) {
	if(!checkBox.isChecked()) {
	 ...
	 try {
	  ...
	  Toast.makeText(SysLogActivity.this, R.string.SysLogLineNumberprompt, Toast.LENGTH_SHORT).show();
	  return false;
	 }
   ...
   protected void onPostExecute(Boolean booleanResult) {
	...
	if(booleanResult) {
	 ...
	 if(inDialog) {
	  ...
	  try {
	   if(!result.equalsIgnoreCase(getString(R.string.SysLogNoLogsFoundprompt))) {
		Date date = new Date();
		String dateString = date.getMonth() + "_" + date.getDay() + "_" + date.getHours() + "_" + date.getMinutes();
		String fileName = user.getUsername() + "_" + menu2[logFile.getSelectedItemPosition()] + "_" + dateString + ".txt";
		target = new FileWriter("/sdcard/Cura/SysLog/" + fileName);
		target.append(result);
		target.flush();
		target.close();
		Toast.makeText(SysLogActivity.this, getString(R.string.logsSaved) + " \"/SysLog/" + fileName + "\"", Toast.LENGTH_LONG).show();
	   }
	   else
		Toast.makeText(SysLogActivity.this, getString(R.string.SysLogNoLogsFoundprompt), Toast.LENGTH_LONG).show();
	  }
	  catch (IOException e) {
	   Toast.makeText(SysLogActivity.this, R.string.logsNotSaved, Toast.LENGTH_LONG).show();
	  }
	 }
	}
	else
   ...
   new AlertDialog.Builder(this).setTitle("Logout Confirmation").setMessage(R.string.logoutConfirmationDialog)
	 .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
	  ...
	  public void onClick(DialogInterface dialog, int which) {
	   ...
	   Intent closeAllActivities = new Intent(SysLogActivity.this, LoginScreenActivity.class);
